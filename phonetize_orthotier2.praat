...
##### Added by Jose M. Lahoz-Bengoechea (2013-01-30)
...
##### Added by Jose M. Lahoz-Bengoechea (2020-12-20)
for int from 2 to nint
l$ = Get label of interval... 'orthoTID' 'int'
if l$ = ""
l$ = "_"
Set interval text... 'orthoTID' 'int' 'l$'
endif
endfor
...
call findtierbyname 'phono_tier$' 0 1
phonoTID=findtierbyname.return
if phonoTID!=0
  if overwrite_phono_tier=1
...
if show_info_window
  ...
  select s4ID
  plus s3ID
##### Added by Jose M. Lahoz-Bengoechea (2011-01-26)
  plus sID+1
##### End of addition
  Remove
  select s2ID
...
##### Modification added by Jose M. Lahoz-Bengoechea (2012-07-08)
select s2ID
Remove
##### End of modification
  select sID
...
select 's2ID'
Remove
##### Modified by Jose M. Lahoz-Bengoechea (2020-12-20)
  select 'tgID'
  Remove tier... 'orthoTID'
  ortho2TID = ortho2TID-1
  Set tier name... 'ortho2TID' 'orthotiername$'
  call findtierbyname phono 1 1
  phonoTID = findtierbyname.return
